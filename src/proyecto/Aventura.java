...
public class Aventura {
    ...
    public ArrayList<Aventura> listarAventura(){
            Statement statement;
            ResultSet rows;
            Conexion conex=new Conexion();
        ArrayList<Aventura> listaAventura=new ArrayList<>();
        try {
            statement=conex.conectar().createStatement();
            rows=statement.executeQuery("SELECT * FROM aventura");
            while (rows.next()){
                Aventura ave=new Aventura();
                ave.setId(rows.getInt("id"));
                ave.setFecha_de_lanzamiento(rows.getDate("fecha_de_lanzamiento"));
                ave.setDesarrollador(rows.getString("desarrollador"));
                ave.setEditor(rows.getString("editor"));
                ave.setPrecio_de_compra(rows.getDouble("precio_de_compra"));
                ave.setLista_de_generos_id(rows.getInt("lista_de_generos_id"));
                ave.setCompra_id(rows.getInt("compra_id"));
                listaAventura.add(ave);
            }
            conex.desconectar();
    ...
    public String registrarAventura(){
        Conexion conex=new Conexion();
        try {
            String sql="INSERT INTO aventura(titulo, fecha_de_lanzamiento, desarrollador, editor, precio_de_compra, lista_de_generos_id, compra_id)";
    ...
    public String actualizarAventura(){
        Conexion conex=new Conexion();
        try {
            String sql="UPDATE aventura SET titulo=?, fecha_de_lanzamiento=?, desarrollador=?, editor=?, precio_de_compra=?, lista_de_generos_id=?, compra_id=? WHERE id=?";
            PreparedStatement preparedStatement=conex.conectar().prepareStatement(sql);
            preparedStatement.setString(1, this.titulo);
            preparedStatement.setDate(2, (java.sql.Date) this.fecha_de_lanzamiento);
            preparedStatement.setString(3, this.desarrollador);
    ...
    public String eliminarAventura(){
        Conexion conex=new Conexion();
        try {
            String sql="DELETE FROM aventura WHERE id=?";
            PreparedStatement preparedStatement=conex.conectar().prepareStatement(sql);
