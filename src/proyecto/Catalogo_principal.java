...
public class Catalogo_principal {
    private int id;
    ...
    public ArrayList<Catalogo_principal> listaCatalogoP(){
            Statement statement;
            ResultSet rows;
            Conexion conex=new Conexion();
        ArrayList<Catalogo_principal> listaCatalogoP=new ArrayList<>();
        try {
            statement=conex.conectar().createStatement();
            rows=statement.executeQuery("SELECT * FROM catalogo_principal");
            while (rows.next()){
                Catalogo_principal ctp=new Catalogo_principal();
                ...
                ctp.setLista_de_generos_id(rows.getInt("lista_de_generos_id"));
                ctp.setCompra_id(rows.getInt("compra_id"));
                listaCatalogoP.add(ctp);
            }
            conex.desconectar();
            return listaCatalogoP;
        }catch (Exception e){
    ...
    public String eliminarCatalogoP(){
        Conexion conex=new Conexion();
        try {
            String sql="DELETE FROM catalogo_principal WHERE id=?";
            PreparedStatement preparedStatement=conex.conectar().prepareStatement(sql);
