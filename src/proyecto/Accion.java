...
public class Accion {
    ...
    public ArrayList<Accion> listarAccion(){
            Statement statement;
            ResultSet rows;
        ...
        try {
            ...
            rows=statement.executeQuery("SELECT * FROM accion");
            while (rows.next()){
                Accion acc=new Accion();
                acc.setId(rows.getInt("id"));
                acc.setTitulo(rows.getString("titulo"));
                acc.setFecha_de_lanzamiento(rows.getDate("fecha_de_lanzamiento"));
                acc.setDesarrollador(rows.getString("desarrollador"));
                acc.setEditor(rows.getString("editor"));
                acc.setPrecio_de_compra(rows.getDouble("precio_de_compra"));
                acc.setLista_de_generos_id(rows.getInt("lista_de_generos_id"));
                acc.setCompra_id(rows.getInt("compra_id"));
                listaAccion.add(acc);
            }
            conex.desconectar();
            return listaAccion;
        }catch (Exception e){
            System.out.println("Error al recuperar los registror");
        }
        return listaAccion;
    }
    public String registrarAccion(){
        Conexion conex=new Conexion();
        try {
            String sql="INSERT INTO accion(titulo, fecha_de_lanzamiento, desarrollador, editor, precio_de_compra, lista_de_generos_id, compra_id)";
            ...
            conex.desconectar();
            return "Filas insertadas: "+rowInserted;
        }catch (Exception e){
            e.printStackTrace();
            return e.getMessage();
        }
    }
    public String actulizarAccion(){
        Conexion conex=new Conexion();
        try {
            String sql="UPDATE accion SET titulo=?, fecha_de_lanzamiento=?, desarrollador=?, editor=?, precio_de_compra=?, lista_de_generos_id=?, compra_id=? WHERE id=?";
            PreparedStatement preparedStatement=conex.conectar().prepareStatement(sql);
            preparedStatement.setString(1, this.titulo);
            preparedStatement.setDate(2, (java.sql.Date) this.fecha_de_lanzamiento);
            preparedStatement.setString(3, this.desarrollador);
    ...
    public String eliminarAccion(){
        Conexion conex=new Conexion();
        try {
            String sql="DELETE FROM accion WHERE id=?";
            PreparedStatement preparedStatement=conex.conectar().prepareStatement(sql);
